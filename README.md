...
| PALSAR HV backscatter | Does ALOS-2 PALSAR HV gamma0 (L-band SAR, sensitive to woody biomass/volume scattering) add signal beyond CHMv2 height + ESA cover fractions? | **Adopted** (19-species median ΔSpearman +0.0004, 16/3 wins, Wilcoxon p=0.0003; ΔDevExpl +0.0008, 16/3, p=0.004; ΔMAE −0.0002, 16/3, p=0.008; ΔRMSE n.s. Biggest gains in forest-interior species — Lyrebird/Whipbird/Rose Robin +0.002–0.004 dev expl. Negative controls (Pacific Black Duck, Welcome Swallow) correctly show no gain. Now wired into `prepare_covariates()` with cache key v7.) | `analysis/palsar/test_palsar_hv_variant.R`, `covariate_tests/palsar_hv_2020/` |
